...
func TestDDL1(t *testing.T) {
	validSQL := []struct {
		...
		// For issue: https://github.com/gogmod/mysql-caddy/pkg/radon/issues/486
		{
			input: "create table test.t (\n" +
				"	`id` int primary key,\n" +
				"	`name` varchar(10)\n" +
...
func TestDDL1ParseError(t *testing.T) {
	invalidSQL := []struct {
		...
		// test some non_reserved_keyword moved to reserved_keyword, issue:https://github.com/gogmod/mysql-caddy/pkg/radon/pull/496
